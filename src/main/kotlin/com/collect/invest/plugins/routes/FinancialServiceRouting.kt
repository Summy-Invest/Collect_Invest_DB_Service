...
fun Route.financialRotes(walletsDao: WalletsDao){


    put("financialService/updateStatus/{id}") {
        try {
    ...
    put("financialService/topupBalance/{id}") {
        try {
            val id = call.parameters["id"]?.toLongOrNull()
    ...
    put("financialService/withdrawBalance/{id}") {
        try {
            val id = call.parameters["id"]?.toLongOrNull()
            val amount: Int = call.receive<Int>()
            if (id != null) {
    ...
    get("financialService/getWallet/{id}") {
        try {
            val id = call.parameters["id"]?.toLongOrNull()
            if (id != null) {
